...
namespace AlanJuden.MvcReportViewer
{
	public abstract class ReportController : Controller
	{
		...
		public JsonResult ViewReportPage(string reportPath, int? page = 0)
		{
            ...
            model.Encoding = System.Text.Encoding.UTF8;
        ...
        public List<ReportParameterInfo> getParameters(string reportPath)
        {
            var model = this.GetReportViewerModel(Request);
            model.ViewMode = ReportViewModes.View;
            model.ReportPath = reportPath;

            return AlanJuden.MvcReportViewer.CoreHtmlHelpers.getReportParameters(model);
        }

        public ReportService.CatalogItem[] ReloadCatalogItems(string catalogPath, bool Recursive)
        {
            var model = this.GetReportViewerModel(Request);
            model.ViewMode = ReportViewModes.View;

            return AlanJuden.MvcReportViewer.ReportServiceHelpers.GetCatalogTreeView(model, catalogPath, Recursive);
        }
